...
class DieticianConversationService extends BaseApi {
  ...
  Future<dynamic> storeMessage(String message, File? file,
    ...
    if (file == null) {
      ...
      var url = '${dotenv.env['BASE_URL']}/api/dietician/chats/store';
      print(url);
  ...
  Future<dynamic> loadMoreMessages(
    ...
    return await api.httpGet('dietician/chats/$userId?page=$page',
        token: token);
  }

  Future<dynamic> readMessage(
      {required String senderId, required String token}) async {
    return await api.httpPost('dietician/chats/read',
