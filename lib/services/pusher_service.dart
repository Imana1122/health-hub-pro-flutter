...
class PusherService {
  static const DieticianChatModel _emptyModel = DieticianChatModel.empty();

  Future<dynamic> getMessagesForUser(
      {required String channelName,
      required ConversationProvider convProvider,
      DieticianChatModel model = _emptyModel,
      String token = ''}) async {
      ...
      await pusher.init(
          ...
          onEvent: (PusherEvent event) {
            ...
            if (jsonData.containsKey('chatMessage')) {
              ...
              if (model != _emptyModel && token != '') {
                if (model.id == chatMessage['sender_id']) {
                  convProvider.readMessages(senderId: model.id, token: token);
                }
              }

              print('Received event: ${chatMessage['sender_id']}');
            } else {
              print('No chatMessage found in event data.');
            }
          },
